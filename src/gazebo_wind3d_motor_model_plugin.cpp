...
namespace gazebo {
    ...
    void GazeboWind3DMotorModel::InitializeParams() {
    }

    void GazeboWind3DMotorModel::Publish() {
    ...
    void GazeboWind3DMotorModel::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
        ...
        updateConnection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&GazeboWind3DMotorModel::OnUpdate, this, _1));

        command_sub_ = node_handle_->Subscribe<mav_msgs::msgs::CommandMotorSpeed>("~/" + model_->GetName() + command_sub_topic_, &GazeboWind3DMotorModel::VelocityCallback, this);
        //std::cout << "[gazebo_motor_model]: Subscribe to gz topic: "<< motor_failure_sub_topic_ << std::endl;
        motor_failure_sub_ = node_handle_->Subscribe<msgs::Int>(motor_failure_sub_topic_, &GazeboWind3DMotorModel::MotorFailureCallback, this);
        // FIXME: Commented out to prevent warnings about queue limit reached.
        //motor_velocity_pub_ = node_handle_->Advertise<std_msgs::msgs::Float>("~/" + model_->GetName() + motor_speed_pub_topic_, 1);
        wind_sub_ = node_handle_->Subscribe("~/" + wind_sub_topic_, &GazeboWind3DMotorModel::WindVelocityCallback, this);
    ...
    void GazeboWind3DMotorModel::VelocityCallback(CommandMotorSpeedPtr & rot_velocities) {
    ...
    void GazeboWind3DMotorModel::UpdateForcesAndMoments() {
    ...
    void GazeboWind3DMotorModel::UpdateMotorFail() {
